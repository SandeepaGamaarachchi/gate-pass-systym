...
const getPending = async (req, res) => {
  try {
    const pendingStatuses = await Status.find({ beforeStatus: 4, afterStatus: null})
      .populate('request')
      .exec();
...
const getApproved = async (req, res) => {
  try {
    const approvedRequests = await Status.find({ afterStatus: 5 })
      .populate('request')
      .exec();
...
const getRejected = async (req, res) => {
  try {
    const rejectedRequests = await Status.find({ afterStatus: 6 })
    .populate('request')
    .exec();
...
const updateApproved = async (req, res) => {
  try {
    ...
    const updatedStatus = await Status.findOneAndUpdate(
      { referenceNumber, beforeStatus: 4 },
      {
        afterStatus: 5,
        comment,
      },
    ...
    if (updatedStatus.request) {
      ...
      updatedStatus.request.status = 7;
...
const updateRejected = async (req, res) => {
  try {
    ...
    const updatedStatus = await Status.findOneAndUpdate(
      { referenceNumber: req.params.referenceNumber, beforeStatus: 4},
      {
        afterStatus: 6,
        comment: comment, 
      },
