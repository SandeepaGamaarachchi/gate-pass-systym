...
const getPending = async (req, res) => {
  try {
    const pendingStatuses = await Status.find({ beforeStatus: 10, afterStatus: null})
      .populate('request')
      .exec();
...
const getApproved = async (req, res) => {
  try {
    const approvedRequests = await Status.find({ afterStatus: 11 })
      .populate('request')
      .exec();
...
const getRejected = async (req, res) => {
  try {
    const rejectedRequests = await Status.find({ afterStatus: 12 })
    .populate('request')
    .exec();
...
const updateApproved = async (req, res) => {
  try {
    const { comment, unloadingDetails } = req.body;
...
const updateRejected = async (req, res) => {
  try {
    ...
    const updatedStatus = await Status.findOneAndUpdate(
      { referenceNumber: req.params.referenceNumber, beforeStatus: 10},
      {
        afterStatus: 12,
        comment: comment, 
      },
