...
const getPending = async (req, res) => {
  ...
  try {
    const pendingStatuses = await Status.find({ beforeStatus: 1, afterStatus: null })
      .populate('request')
      .exec();
...
const getApproved = async (req, res) => {
  try {
    const approvedRequests = await Status.find({ afterStatus: 2 })
      .populate('request')
      .exec();
...
const getRejected = async (req, res) => {
  try {
    const rejectedRequests = await Status.find({ afterStatus: 3 })
    .populate('request')
    .exec();
...
const updateApproved = async (req, res) => {
  try {
    ...
    const updatedStatus = await Status.findOneAndUpdate(
      { referenceNumber, beforeStatus: 1 },
      {
        afterStatus: 2,
        comment,
      },
...
const updateRejected = async (req, res) => {
  try {
    const {comment} = req.body;
    const updatedStatus = await Status.findOneAndUpdate(
      { referenceNumber: req.params.referenceNumber, beforeStatus: 1},
      {
        afterStatus: 3,
        comment: comment, 
      },
